...
class APIFlask(APIScaffold, Flask):
    ...
    *Version changed: 0.9.0*
                            ...
                            if (new_headers := value.get('headers')) is not None:
                                ...
                                if not isinstance(new_headers, dict) or _is_headers_schema_dict(
                                    ...
                                    new_headers = self._make_response_headers(
                                        t.cast('SchemaType', new_headers), spec
                                    )
    ...
    def _make_response_headers(self, headers_schema: SchemaType, spec: APISpec) -> dict[str, t.Any]:
        ...
        Arguments:
            headers_schema: The schema describing the response headers.
            spec: The `APISpec` object, passed on so that nested schemas
                referenced by the headers are registered.
